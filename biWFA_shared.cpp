...
#include "headers/biWFA_shared.h"
#include <cstdlib>
#include <chrono>
...
__device__ void extend_max(bool *finish, const int score, int32_t *max_ak, wf_components_t *wf, const int max_score_scope, const int alignment_k, const int32_t alignment_offset, const int pattern_len) {
    if (wf->mwavefronts[score%num_wavefronts].null) {
        ...
        for (k = wf->mwavefronts[score%num_wavefronts].lo; k <= wf->mwavefronts[score%num_wavefronts].hi; ++k) {
            int32_t offset = wf->mwavefronts[score%num_wavefronts].offsets[k+wf_length/2];
            ...
            wf->mwavefronts[score%num_wavefronts].offsets[k+wf_length/2] = offset;

            int32_t antidiag = (2 * wf->mwavefronts[score%num_wavefronts].offsets[k+wf_length/2]) - k;
        ...
        *max_ak = max_antidiag_loc;
        ...
        if (wf->mwavefronts[score%num_wavefronts].lo > alignment_k || alignment_k > wf->mwavefronts[score%num_wavefronts].hi) {
            ...
            int32_t moffset = wf->mwavefronts[score%num_wavefronts].offsets[alignment_k+wf_length/2];
...
__device__ void extend(bool *finish, const int score, wf_components_t *wf, const int max_score_scope, const int alignment_k, const int32_t alignment_offset, const int pattern_len) {
    if (wf->mwavefronts[score%num_wavefronts].null) {
        if (wf->alignment.num_null_steps > max_score_scope) {
            *finish = true;
        } else {
...
__device__ void nextWF(int *score, wf_components_t *wf, const bool forward, const int max_score_scope, const int text_len, const int pattern_len) {
    ...
    wf_t in_mwavefront_misms = (mismatch < 0 || wf->mwavefronts[mismatch%num_wavefronts].null) ? wf->wavefront_null : wf->mwavefronts[mismatch%num_wavefronts];
    wf_t in_mwavefront_open = (gap_open < 0 || wf->mwavefronts[gap_open%num_wavefronts].null) ? wf->wavefront_null : wf->mwavefronts[gap_open%num_wavefronts];
    wf_t in_iwavefront_ext = (gap_extend < 0 || wf->iwavefronts[gap_extend%num_wavefronts].null) ? wf->wavefront_null : wf->iwavefronts[gap_extend%num_wavefronts];
    wf_t in_dwavefront_ext = (gap_extend < 0 || wf->dwavefronts[gap_extend%num_wavefronts].null) ? wf->wavefront_null : wf->dwavefronts[gap_extend%num_wavefronts];
    ...
    if (in_mwavefront_misms.null && in_mwavefront_open.null && in_iwavefront_ext.null && in_dwavefront_ext.null) {
        ...
        if (!m_misms_null) {
            ...
            if (in_mwavefront_misms.wf_elements_init_max >= hi) {
                ...
                for (k = max_init + 1; k <= hi; ++k) {
                    in_mwavefront_misms.offsets[k+wf_length/2] = OFFSET_NULL;
                }
                // Set new maximum
        ...
        if (!m_gap_null) {
            ...
            if (in_mwavefront_open.wf_elements_init_max >= hi + 1) {
                ...
                for (k = max_init + 1; k <= hi + 1; ++k) {
                    in_mwavefront_open.offsets[k+wf_length/2] = OFFSET_NULL;
                }
                // Set new maximum
        ...
        if (!i_ext_null) {
            ...
            if (in_iwavefront_ext.wf_elements_init_max >= hi) {
                ...
                int k;
                for (k = max_init + 1; k <= hi; ++k) {
                    in_iwavefront_ext.offsets[k+wf_length/2] = OFFSET_NULL;
                }
                // Set new maximum
        ...
        if (!d_ext_null) {
            ...
            if (in_dwavefront_ext.wf_elements_init_max >= hi + 1) {
                ...
                int k;
                for (k = max_init + 1; k <= hi + 1; ++k) {
                    in_dwavefront_ext.offsets[k+wf_length/2] = OFFSET_NULL;
                }
                // Set new maximum
        ...
        for (int i = lo; i <= hi; i += hipBlockDim_x) {
            ...
            if (idx <= hi) {
                // Update I1
                int32_t ins_o = in_mwavefront_open.offsets[idx - 1+wf_length/2];
                int32_t ins_e = in_iwavefront_ext.offsets[idx - 1+wf_length/2];
                int32_t ins = MAX(ins_o, ins_e) + 1;
                wf->iwavefronts[score_mod].offsets[idx+wf_length/2] = ins;

                // Update D1
                int32_t del_o = in_mwavefront_open.offsets[idx + 1+wf_length/2];
                int32_t del_e = in_dwavefront_ext.offsets[idx + 1+wf_length/2];
                int32_t del = MAX(del_o, del_e);
                wf->dwavefronts[score_mod].offsets[idx+wf_length/2] = del;

                // Update M
                int32_t misms = in_mwavefront_misms.offsets[idx+wf_length/2] + 1;
                ...
                wf->mwavefronts[score_mod].offsets[idx+wf_length/2] = max;
            }
        }
        __syncthreads();

        // wavefront_compute_process_ends()
        if (wf->mwavefronts[score_mod].null == false) {
            // wavefront_compute_trim_ends()
            int k;
            int lo = wf->mwavefronts[score_mod].lo;
            for (k = wf->mwavefronts[score_mod].hi; k >= lo; --k) {
                // Fetch offset
                int32_t offset = wf->mwavefronts[score_mod].offsets[k+wf_length/2];
                // Check boundaries
        ...
        if (wf->iwavefronts[score_mod].null == false) {
            // wavefront_compute_trim_ends()
            int k;
            int lo = wf->iwavefronts[score_mod].lo;
            for (k = wf->iwavefronts[score_mod].hi; k >= lo; --k) {
                // Fetch offset
                int32_t offset = wf->iwavefronts[score_mod].offsets[k+wf_length/2];
                // Check boundaries
                uint32_t h = offset; // Make unsigned to avoid checking negative
                uint32_t v = offset - k; // Make unsigned to avoid checking negative
                if (h <= text_len && v <= pattern_len) break;
        ...
        if (wf->dwavefronts[score_mod].null == false) {
            // wavefront_compute_trim_ends()
            int k;
            int lo = wf->dwavefronts[score_mod].lo;
            for (k = wf->dwavefronts[score_mod].hi; k >= lo ; --k) {
                // Fetch offset
                int32_t offset = wf->dwavefronts[score_mod].offsets[k+wf_length/2];
                // Check boundaries
                uint32_t h = offset; // Make unsigned to avoid checking negative
                uint32_t v = offset - k; // Make unsigned to avoid checking negative
                if (h <= text_len && v <= pattern_len) break;
...
__device__ void breakpoint_indel2indel(const int score_0, const int score_1, const wf_t *dwf_0, const wf_t *dwf_1, int *breakpoint_score, const int text_len, const int pattern_len) {
    ...
    for (k_0 = max_lo; k_0 <= min_hi; k_0++) {
        ...
        // Fetch offsets
        int dh_0 = dwf_0->offsets[k_0+wf_length/2];
        int dh_1 = dwf_1->offsets[k_1+wf_length/2];
...
__device__ void breakpoint_m2m(const int score_0, const int score_1, const wf_t *mwf_0, const wf_t *mwf_1, int *breakpoint_score, const int text_len, const int pattern_len) {
    ...
    for (k_0 = max_lo; k_0 <= min_hi; k_0++) {
        ...
        // Fetch offsets
        const int mh_0 = mwf_0->offsets[k_0+wf_length/2];
        const int mh_1 = mwf_1->offsets[k_1+wf_length/2];
...
__device__ void overlap(const int score_0, wf_components_t *wf_0, const int score_1, wf_components_t *wf_1, const int max_score_scope, int *breakpoint_score, const int text_len, const int pattern_len) {
    // Fetch wavefront-0
...
__global__ void biWFA(char *pattern_f_g, char *text_f_g, char *pattern_r_g, char *text_r_g, int *breakpoint_score_g, const int lo_g, const int hi_g,
                      const int pattern_len, const int text_len, const int max_score_scope) {
    int lo, hi;
    ...
    for (int i = 0; i < num_wavefronts * wf_length; i += hipBlockDim_x) {
        if (i + hipThreadIdx_x < num_wavefronts * wf_length) {
            wf_f.mwavefronts[(i + hipThreadIdx_x) / wf_length].offsets[(i + hipThreadIdx_x) % wf_length] = OFFSET_NULL;
            wf_f.iwavefronts[(i + hipThreadIdx_x) / wf_length].offsets[(i + hipThreadIdx_x) % wf_length] = OFFSET_NULL;
            wf_f.dwavefronts[(i + hipThreadIdx_x) / wf_length].offsets[(i + hipThreadIdx_x) % wf_length] = OFFSET_NULL;
            wf_r.mwavefronts[(i + hipThreadIdx_x) / wf_length].offsets[(i + hipThreadIdx_x) % wf_length] = OFFSET_NULL;
            wf_r.iwavefronts[(i + hipThreadIdx_x) / wf_length].offsets[(i + hipThreadIdx_x) % wf_length] = OFFSET_NULL;
            wf_r.dwavefronts[(i + hipThreadIdx_x) / wf_length].offsets[(i + hipThreadIdx_x) % wf_length] = OFFSET_NULL;
        }
    }

    for (int i = 0; i < wf_length; i += hipBlockDim_x) {
        if (i + hipThreadIdx_x < wf_length) {
            wf_f.wavefront_null.offsets[(i + hipThreadIdx_x)] = OFFSET_NULL;
            wf_r.wavefront_null.offsets[(i + hipThreadIdx_x)] = OFFSET_NULL;
        }
    }

    for (int i = 0; i < num_wavefronts; i += hipBlockDim_x) {
        if (i + hipThreadIdx_x < num_wavefronts) {
            wf_f.mwavefronts[i + hipThreadIdx_x].null = true;
            wf_f.mwavefronts[i + hipThreadIdx_x].lo = 0;
            wf_f.mwavefronts[i + hipThreadIdx_x].hi = 0;
            wf_f.mwavefronts[i + hipThreadIdx_x].wf_elements_init_max = 0;
            wf_f.mwavefronts[i + hipThreadIdx_x].wf_elements_init_min = 0;
            wf_r.mwavefronts[i + hipThreadIdx_x].null = true;
            wf_r.mwavefronts[i + hipThreadIdx_x].lo = 0;
            wf_r.mwavefronts[i + hipThreadIdx_x].hi = 0;
            wf_r.mwavefronts[i + hipThreadIdx_x].wf_elements_init_max = 0;
            wf_r.mwavefronts[i + hipThreadIdx_x].wf_elements_init_min = 0;

            wf_f.iwavefronts[i + hipThreadIdx_x].null = true;
            wf_f.iwavefronts[i + hipThreadIdx_x].lo = 0;
            wf_f.iwavefronts[i + hipThreadIdx_x].hi = 0;
            wf_f.iwavefronts[i + hipThreadIdx_x].wf_elements_init_max = 0;
            wf_f.iwavefronts[i + hipThreadIdx_x].wf_elements_init_min = 0;
            wf_r.iwavefronts[i + hipThreadIdx_x].null = true;
            wf_r.iwavefronts[i + hipThreadIdx_x].lo = 0;
            wf_r.iwavefronts[i + hipThreadIdx_x].hi = 0;
            wf_r.iwavefronts[i + hipThreadIdx_x].wf_elements_init_max = 0;
            wf_r.iwavefronts[i + hipThreadIdx_x].wf_elements_init_min = 0;

            wf_f.dwavefronts[i + hipThreadIdx_x].null = true;
            wf_f.dwavefronts[i + hipThreadIdx_x].lo = 0;
            wf_f.dwavefronts[i + hipThreadIdx_x].hi = 0;
            wf_f.dwavefronts[i + hipThreadIdx_x].wf_elements_init_max = 0;
            wf_f.dwavefronts[i + hipThreadIdx_x].wf_elements_init_min = 0;
            wf_r.dwavefronts[i + hipThreadIdx_x].null = true;
            wf_r.dwavefronts[i + hipThreadIdx_x].lo = 0;
            wf_r.dwavefronts[i + hipThreadIdx_x].hi = 0;
            wf_r.dwavefronts[i + hipThreadIdx_x].wf_elements_init_max = 0;
            wf_r.dwavefronts[i + hipThreadIdx_x].wf_elements_init_min = 0;
        }
    }
    __syncthreads();

    wf_f.alignment.pattern = pattern_f_g + (pattern_len * hipBlockIdx_x);
    wf_f.alignment.text = text_f_g + (text_len * hipBlockIdx_x);
    wf_f.alignment.historic_max_hi = 0;
    wf_f.alignment.historic_min_lo = 0;
    wf_f.alignment.num_null_steps = 0;

    wf_r.alignment.pattern = pattern_r_g + (pattern_len * hipBlockIdx_x);
    wf_r.alignment.text = text_r_g + (text_len * hipBlockIdx_x);
    wf_r.alignment.historic_max_hi = 0;
    wf_r.alignment.historic_min_lo = 0;
    ...
    wf_f.mwavefronts[0].offsets[0+wf_length/2] = 0;
    ...
    wf_r.mwavefronts[0].offsets[0+wf_length/2] = 0;
...
int main(int argc, char *argv[]) {
    ...
    biWFA<<<blocksPerGrid, threadsPerBlock>>>(pattern_f_g, text_f_g, pattern_r_g, text_r_g, breakpoint_score_g, lo, hi, pattern_len, text_len, max_score_scope);
